...
export const PublicProfile = () => {
  ...
  const handleFollowToggle = async () => {
    try {
      if (isFollowing) {
        // Unfollow the user
        await api.post(`/follows/${profileData.id}/unfollow/`);
        ...
        // Follow the user
        await api.post('/follows/', { followee: profileData.id });
        setIsFollowing(true);
        setProfileData(prev => ({
          ...prev,
  ...
  const handleMessageUser = () => {
    navigation.navigate('CreateDirectChat', { userId: profileData.id, username: profileData.username });
  };
